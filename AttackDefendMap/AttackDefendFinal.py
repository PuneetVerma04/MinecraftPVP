...
class SwingDetector:
    ...
    def __init__(self, config: GameConfig):
        ...
        self.FAST_MOVEMENT_THRESHOLD = 0.05  # Increased from 0.03
        self.SWING_DETECTION_THRESHOLD = 1    # Reduced from 2
    ...
    def detect_swing(self, landmarks: List[List[float]], shield_state: str) -> bool:
        ...
        Simplified state machine with faster response
        """
        ...
        # Simplified state machine for faster swing detection
        swing_detected = False
        
        if self.swing_state == "ready":
            self.reset_click_state()
            # Look for fast downward movement
            ...
            if pos_change < -self.FAST_MOVEMENT_THRESHOLD:
                ...
                if self.state_counter >= self.SWING_DETECTION_THRESHOLD:
                    ...
                    self.state_counter = 0
                    self.total_swings += 1
                    self.cooldown_counter = max(3, self.config.COOLDOWN_FRAMES // 2)  # Reduced cooldown
                    swing_detected = True
            else:
                # Return to ready if swing motion stops
                self.swing_state = "ready"
                self.state_counter = 0
...
class ShieldDetector:
    ...
    @staticmethod
    def filter_non_left_hand(hand_landmarks, handedness) -> Optional[mp.solutions.hands.HandLandmark]:
        """
        Ensure only the left hand is processed by filtering out the right hand landmarks.
        """
        ...
        if results_hands.multi_hand_landmarks and results_hands.multi_handedness:
            for hand_landmarks, handedness in zip(results_hands.multi_hand_landmarks, results_hands.multi_handedness):
                if shield_detector.is_left_hand(handedness):
                    ...
                    mp_drawing.draw_landmarks(frame, hand_landmarks, mp_hands.HAND_CONNECTIONS, mp_drawing.DrawingSpec(color=color, thickness=2), mp_drawing.DrawingSpec(color=color, thickness=2))
