...
class SwingDetector:
    ...
    def get_wrist_position(self, landmarks: List[List[float]]) -> float:
        ...
        if not landmarks:
            return 0
        # Using RIGHT_SHOULDER (12) and RIGHT_WRIST (16) landmarks
        right_shoulder = np.array([landmarks[12][1]])  # Changed from 11 to 12 for right shoulder
        right_wrist = np.array([landmarks[16][1]])
        return float(right_shoulder - right_wrist)
    
    def detect_swing(self, landmarks: List[List[float]]) -> bool:
...
class ShieldDetector:
    ...
    def update_shield_state(self, current_shield_up: bool) -> str:
        ...
        self.shield_state = "Up" if sum(self.shield_buffer) > len(self.shield_buffer) / 2 else "Down"
...
def main():
    ...
    hands = mp_hands.Hands(min_detection_confidence=0.7, min_tracking_confidence=0.7, max_num_hands=2)  # Changed to 2 hands
    ...
    while cap.isOpened():
        ...
        # Process pose for right hand attack detection
        results_pose = pose.process(rgb_frame)
        swing_detected = False
        if results_pose.pose_landmarks:
            landmarks = [[lm.x, lm.y, lm.z] for lm in results_pose.pose_landmarks.landmark]
            swing_detected = swing_detector.detect_swing(landmarks)
            mp_drawing.draw_landmarks(frame, results_pose.pose_landmarks, mp_pose.POSE_CONNECTIONS)
        
        # Process hands for left hand shield detection
        ...
        if results_hands.multi_hand_landmarks and results_hands.multi_handedness:
            for hand_landmarks, handedness in zip(results_hands.multi_hand_landmarks, 
                ...
                if shield_detector.is_left_hand(handedness):  # Fixed: pass individual handedness
