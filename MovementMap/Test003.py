...
def map_action(state, current_actions):
...
def stop_action(state, current_actions):
    keyboard = KeyboardController()
...
def main():
    ...
    while cap.isOpened():
        ...
        if detector.state is not None:
            ...
            map_action(detector.state, current_actions)  # Map detected movement to game controls
            ...
            if detector.state != detector.last_movement and detector.last_movement is not None:
                stop_action(detector.last_movement, current_actions)
    ...
    for action in current_actions.keys():
        stop_action(action, current_actions)
