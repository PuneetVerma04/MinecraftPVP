...
VERTICAL_MOVEMENT_THRESHOLD = 0.03  # Sensitivity for vertical movement
SWING_STATE_THRESHOLD = 2  # Reduced frames needed to confirm state
COOLDOWN_FRAMES = 8  # Reduced cooldown for faster detection
SHOULDER_HEIGHT_THRESHOLD = 0.05  # Threshold above right shoulder
...
def get_wrist_position_relative_to_shoulder(landmarks):
    """Get wrist position relative to right shoulder"""
    if not landmarks:
        return 0
    
    right_shoulder = np.array([landmarks[11][1]])  # Right shoulder Y coordinate (using mark 11)
    right_wrist = np.array([landmarks[16][1]])     # Right wrist Y coordinate
    return float(right_shoulder - right_wrist)  # Positive when wrist is above shoulder
...
def detect_swing_motion(landmark_history):
    ...
    # State machine logic with right shoulder reference
    if swing_state == "ready":
        if position_change > VERTICAL_MOVEMENT_THRESHOLD:  # Moving up
            state_counter += 1
            if state_counter >= SWING_STATE_THRESHOLD:
                swing_state = "lifting"
                state_counter = 0
        else:
            state_counter = 0  # Reset immediately for faster response

    elif swing_state == "lifting":
        # Check if wrist is at or slightly above right shoulder
        if -SHOULDER_HEIGHT_THRESHOLD <= current_position <= 0.2:  
            if position_change < -VERTICAL_MOVEMENT_THRESHOLD:  # Starting to move down
        ...
        if position_change < -VERTICAL_MOVEMENT_THRESHOLD:  # Downward motion
            state_counter += 1
            if state_counter >= SWING_STATE_THRESHOLD:
                swing_state = "ready"
                state_counter = 0
