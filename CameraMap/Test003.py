...
class SensitivitySettings:
    x: float = 6.0  # Horizontal sensitivity
    y: float = 6.0  # Vertical sensitivity
    acceleration: float = 1.2
    dead_zone: float = 0.02  # Minimum movement threshold
    movement_scale: float = 50.0  # Scale factor for mouse events
...
class FaceTracker:
    ...
    def move_camera(self, dx: int, dy: int):
        """Send direct mouse movement input"""
        if abs(dx) > 0 or abs(dy) > 0:
            # Use win32api for direct mouse movement
        ...
        if results.multi_face_landmarks:
            ...
            if self.calibration_frames < self.CALIBRATION_DURATION:
                ...
                if self.calibration_center is not None:
                    ...
                    cv2.putText(frame, f"Move: {dx}, {dy}", (10, frame.shape[0] - 30),
...
def main():
    ...
    print("2. Press F11 or enter fullscreen mode")
    print("3. Make sure mouse cursor is hidden/locked in game")
    print("4. Hold your head steady for initial calibration")
    print("5. Move your head slightly to control the camera")
