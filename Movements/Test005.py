...
class EnhancedMovementDetector:
    def __init__(self):
        ...
        self.VERTICAL_THRESHOLD = 30    # For forward/backward movements
        self.HORIZONTAL_THRESHOLD = 20  # For left/right movements
...
def main():
    ...
    cap = cv2.VideoCapture("http://192.168.137.168:8080/video")  # Use DroidCam URL

    # Set video capture for portrait mode (portrait resolution)
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 480)   # Set width for portrait mode 
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 640)   # Set height for portrait mode 
    ...
    while cap.isOpened():
        ...
        # Rotate the frame to display in portrait orientation if needed.
